...
public class MallPcExceptionHandler {
    ...
    public Object returnErrorCode(Throwable exception) {
        if (exception instanceof MissingServletRequestParameterException) {
            return Result.failure(MallExceptionCode.REQUIRED_PARAM_MISSING.getCode(), MallExceptionCode.REQUIRED_PARAM_MISSING.getMsg());
        } else if (exception instanceof IllegalArgumentException) {
            return Result.failure(MallExceptionCode.PARAM_ERROR.getCode(), exception.getMessage());
        } else if (exception instanceof BindException) {
            FieldError fieldError = ((BindException) exception).getBindingResult().getFieldError();
            return Result.failure(MallExceptionCode.PARAM_ERROR.getCode(), fieldError.getDefaultMessage());
            ...
            return Result.failure(MallExceptionCode.SYSTEM_ERROR);
        }
    }
