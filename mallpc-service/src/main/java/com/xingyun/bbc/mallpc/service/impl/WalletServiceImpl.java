...
import static com.xingyun.bbc.mallpc.common.enums.OrderPayMent.OrderPayStatusEnum.*;
...
public class WalletServiceImpl implements WalletService {
    ...
    public List<BanksVo> queryBankList() {
        ...
        if (CollectionUtils.isEmpty(result.getData())) {
            throw new BizException(MallPcExceptionCode.BANK_NOT_CONFIG);
        }
    ...
    public Boolean withdraw(@Valid WithdrawDto withdrawDto) {
        ...
        try {
            ...
            Ensure.that(xybbcLock.tryLock(withdrawLockKey, withdrawLockValue, 300)).isTrue(MallPcExceptionCode.WITHDRAW_PROCESSING);
    ...
    private Long withdrawCheck(@Valid WithdrawDto withdrawDto) {
        ...
        if (!this.checkPayPwd(uid)) {
            ...
            throw new BizException(MallPcExceptionCode.USER_PAY_PWD_NOT_SET);
        }
        ...
        if (StringUtil.isBlank(withdrawDto.getAccountNumber()) && StringUtil.isBlank(withdrawDto.getCardNumber())) {
            ...
            throw new BizException(MallPcExceptionCode.WITHDRAW_ACCOUNT_EMPTY);
        }
        ...
        if (balance.compareTo(new BigDecimal("0.00")) <= 0 ||
            ...
            throw new BizException(MallPcExceptionCode.ACCOUNT_BALANCE_INSUFFICIENT);
        }

        String passWord = EncryptUtils.aesDecrypt(withdrawDto.getWithdrawPwd());

        if (StringUtil.isBlank(passWord)) {
            throw new BizException(MallPcExceptionCode.WITHDRAW_PASSWORD_ERROR);
        }
        ...
        if (null == result.getData()) {
            throw new BizException(MallPcExceptionCode.ACCOUNT_NOT_EXIST);
        }

        if (result.getData().getFfreezeWithdraw() < 0) {
            log.warn("提现冻结金额小于0");
            throw new BizException(MallPcExceptionCode.REEZE_WITHDRAW_ERROR);
        }
        ...
        if (Objects.isNull(withdrawRate)) {
            ...
            if (withdrawDto.getWithdrawAmount().compareTo(new BigDecimal("100"))<0){
                throw new BizException(MallPcExceptionCode.LESS_THAN_ONE_RMB);
            }

        //提现费率配置则提现金额不得低于配置金额
        }else if(PriceUtil.toPenny(withdrawRate.getMinimumAmount()).compareTo(withdrawDto.getWithdrawAmount())>0){

            throw new BizException(MallPcExceptionCode.WITHDRAW_LES_MIN_AMOUNT);
        }
    ...
    private User checkUser(Long uid) {
        ...
        if (null == user) {
            throw new BizException(MallPcExceptionCode.NAME_NOT_EXIST);
        }
        ...
        if (user.getFfreezeStatus() != 1) {
            throw new BizException(MallPcExceptionCode.ACCOUNT_FREEZE);
        }

        // 用户状态：1未认证，2 认证中，3 已认证，4未通过
        if (user.getFverifyStatus() != 3) {
            throw new BizException(MallPcExceptionCode.ACCOUNT_NOT_AUTH);
        }

        // 手机号是否验证：0否，1是
        //if (user.getFmoblieIsValid() != 1) throw new BizException(MallPcExceptionCode.ACCOUNT_MOBILE_NOT_VERIFY);

        // 邮箱是否验证：0否，1是
        //if (user.getFmailIsValid() != 1) throw new BizException(MallPcExceptionCode.ACCOUNT_MAIL_NOT_VERIFY);
        return user;
    }
    ...
    private class AccountTrans {
        ...
        public AccountTrans build() {
            ...
            userAccountTrans.setFtransPoundage(PriceUtil.toPenny(feeAmount).longValue());
    ...
    private String getWithdrawKey(WithdrawDto withdrawDto) {
        ...
        return StringUtils.join(Lists.newArrayList(MallPcRedisConstant.ADD_USER_WITHDRAW_LOCK, this.appendKey(withdrawDto), withdrawDto.getUid()), ":");
    }
